...
tnspec_get_sw_variables() {
    local sw_vars="signed_vars cfg bct dtb sku odm cfg_override"
...
nct_upgrade_tnspec() {
    ...
    if [ -z "$spec" ]; then
        ...
        local sn="$(_tnspec nct dump serial -n $nctbin)"
        local hwid=$(tnspec spec list $tnsid -g hw | head -n 1)

        [ -n "$hwid" ] && {
            pr_ok  "FOUND matching board name [$hwid]" "TNSPEC Upgrade: "
            tnspec spec list $tnsid -g hw -v | head -n 1
            TNSPEC_SET_HW="sn=$sn" tnspec nct new $hwid -o $nctbin.tmp &&
...
flash_main() {
    ...
    case $command in
        ...
        register)
            ...
            ;;
        test)
            db_tnspec_register "${command_args[@]}"
            ;;
    esac
...
factory_set_macs() {
    ...
    mac_envs[eth]=BOARD_ETH; mac_descs[eth]="Ethernet (unused)"
