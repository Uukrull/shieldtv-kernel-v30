...
function krebuild()
{
    ...
    local PLATFORM_IS_NEXT=$(get_build_var PLATFORM_IS_NEXT)

    if [ ${PLATFORM_IS_NEXT} != "1" ]; then
...
function _nvflash_sh()
{
    ...
    if [[ "${NV_REQUIRES_EMBEDDED_FOUNDATION}" != true ]]; then
        cp -u $T/vendor/nvidia/build/$FLASH_SH $PRODUCT_OUT/flash.sh

        # WAR - tnspec.py can be missing in some packages.
        cp -u $T/vendor/nvidia/tegra/core/tools/tnspec/$TNSPEC_PY $PRODUCT_OUT/tnspec.py
    fi
