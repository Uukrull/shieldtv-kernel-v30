...
class FWCheck():
    ...
    def check_emmc_fw(self, tag, versions, exception):
        cmd = "adb shell cat /sys/kernel/debug/mmc0/mmc0:0001/ext_csd"
        ...
        if len(result) > 524:
            version_on_board = result[508:508+16]
            ...
            print_info(tag, "fw mismatch, ext_csd on device: %s; length: %d" %(result, len(result)))
    ...
    def check_xusb_fw(self, tag, versions, exception):
        ...
        cmd = "adb shell dmesg | grep 'tegra-xhci: " + matching_phrase + "'"
        result = run_cmd(tag, cmd)
...
def run_setup():
    ...
    result = run_cmd("setup", adb_remount)
    if "remount succeeded" not in result:
        print "adb failed to root and remount. abort."
        return False
